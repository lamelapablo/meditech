...
let listaBotonesVerComentarios = document.querySelectorAll('.btn-ver-comentario');
let modalVerComentarios = document.getElementById('modal-ver-comentarios');
...
listaBotonesVerComentarios.forEach(boton =>{
    boton.addEventListener('click', () =>{
        modalVerComentarios.showModal();
    });
});

btnCerrarModalComentarios.addEventListener('click', () =>{
    modalVerComentarios.close();
})
...
var opcionBoton = 'especialidad';

let listaEspecialidadesTurnos = document.querySelectorAll('.especialidad');
let listaMedicosTurnos = document.querySelectorAll('.medico');
...
listaBotonesBuscarPorTurnos[0].classList.add('set-background'); //se selecciona por default el boton de buscar por especialidad
//se filtra por default por especialidades
filtrar(listaEspecialidadesTurnos)
...
listaBotonesBuscarPorTurnos.forEach(boton =>{
    ...
    boton.addEventListener('click', () =>{
        ...
        let placeholderInput = 'Buscar turno por ' + ultimaPalabra; //genero el nuevo placeholder del input
        inputBuscarTurnos.setAttribute('placeholder', placeholderInput); //cambio el placeholder del input
